...
classes = ['kc', 'lf', 'xb']  # 类别名
TRAIN_RATIO = 90  # 训练集占得比例，80就是占80%

dir1 = r"E:\project\datasets\source-huizong/"  # 要处理的数据的主目录。前面的r不能删，后面的/一定要带上
dir2 = r""  # 要处理的数据的子目录。前面的r不能删，后面的/一定要带上

input_picPath_dir_name = r"img/"  # 输入的图片所在的文件夹的名字。前面的r不能删，后面的/一定要带上
input_xmlPath_dir_name = r"xml/"  # 输入的xml标签所在的文件夹的名字。前面的r不能删，后面的/一定要带上
output_txtPath_dir_name = r"txt/"  # 输出的txt标签所在的文件夹的名字。前面的r不能删，后面的/一定要带上
